...
**When:** Every changed file in the merge request is under `spec/`, and
...
- **Tests cover the change if needed.** New or changed library behavior has a
  matching case under `spec/`. Changes that genuinely need no tests, such
...
- **The public API stays backwards compatible.** Adding new capability is fine:
  ...
  method, a raised minimum supported Ruby version, or a change to the runtime dependencies in
  `Gemfile` or `deepl-rb.gemspec`. This library is published to RubyGems as `deepl-rb`, so a break here
